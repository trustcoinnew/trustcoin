...
int64 nChainStartTime = 1406353616;
...
const string strMessageMagic = "TrustCoin Signed Message:\n";
...
static const int CUTOFF_HEIGHT = 51000;	// Height at the end of 5 weeks
// miner's coin base reward based on nBits
int64 GetProofOfWorkReward(int nHeight, int64 nFees, uint256 prevHash)
{
	int64 nSubsidy = 75000 * COIN;
	...
	nSubsidy += generateMTRandom(seed, 75000) * COIN;
...
const int YEARLY_BLOCKCOUNT = 700800;	// 365 * 2880 (1920)
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    bool fEnforceBIP30 = true; // Always active in TrustCoin
    bool fStrictPayToScriptHash = true; // Always active in TrustCoin
    ...
    // ppcoin: track money supply and trust amount info
    pindex->nTrust = nValueOut - nValueIn + nFees;
...
bool CheckDiskSpace(uint64 nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "TrustCoin", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        // Genesis block
        const char* pszTimestamp = "Jun 15, 2014: The Denver Broncos finally got on the board with a touchdown in the final seconds of the third quarter. But the Seattle Seahawks are dominating the Broncos 36-8";
        CTransaction txNew;
        ...
        block.nVersion = 1;
        block.nTime    = 1406353616;
        block.nBits    = bnProofOfWorkLimit.GetCompact();
        block.nNonce   = 1414993616;
        ...
        assert(block.hashMerkleRoot == uint256("3a4f968a123ab564708d2594f32806c198371a8d3f29824a5959667636189135"));
...
void PrintBlockTree()
{
    ...
    while (!vStack.empty())
    {
        ...
        printf("%d (%u,%u) %s  %08x  %s  trust %7s  tx %"PRIszu"",
            pindex->nHeight,
            pindex->nFile,
            pindex->nBlockPos,
            block.GetHash().ToString().c_str(),
            ...
            FormatMoney(pindex->nTrust).c_str(),
            block.vtx.size());
...
CBlock* CreateNewBlock(CWallet* pwallet, bool fProofOfStake)
{
    ...
    int64 nTrustxFee = MIN_TX_FEE;
    if (mapArgs.count("-trustxfee"))
        ParseMoney(mapArgs["-trustxfee"], nTrustxFee);
    ...
    int64 nFees = 0;
    {
        ...
        while (!vecPriority.empty())
        {
            ...
            if (fSortedByFee && (dFeePerKb < nTrustxFee) && (nBlockSize + nTxSize >= nBlockMinSize))
                continue;
